...
```bash
just check             # the full gate, container-only  (~31s)
just fmt               # ruff rewrites your working tree (bind-mounted), not the image's
DOCKER=0 just check    # opt out: run against the local venv instead  (~24s)
```
...
native path (`DOCKER=0`) is faster — ~24s vs ~31s for `just check`, and ~0.05s vs ~0.6s
for a bare `just lint`, the difference being container startup — but needs the Python
toolchain installed by `just install`.
