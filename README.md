...
| `QUEUE_PROGRESS_MAX_SECS` | | `300.0` | How long the card keeps editing before it settles on "still working" and stops. Not a timeout: the enqueue carries on, and the card is still deleted when it lands. `PLAY_RESOLVE_WAIT_SECS` bounds the wait for a slot, this bounds the editing. Must be at least `QUEUE_PROGRESS_DELAY_SECS` + `QUEUE_PROGRESS_TICK_SECS` (7.5 by default); below that the bot refuses to start. A card always keeps editing for at least its delay plus two ticks, whatever this says |
