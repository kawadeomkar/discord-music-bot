...
12. **Redis eviction policy is `volatile-lru` on purpose.** Three keys carry no TTL
    ...
    `guild:{id}:config` holds a guild's DURABLE choices (debug mode, volume, timezone, idle and alone timeouts) — evicting it silently reverts a setting the guild chose, with no log
...
| `NOW_PLAYING_UPDATE_INTERVAL_SECS` | `3.0` | NP progress-bar edit cadence. Floored at 1.0 and refused non-finite: every playing tick is a real edit on the channel's 5-edits-per-5s bucket |
