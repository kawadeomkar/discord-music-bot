...
| Tests | pytest + pytest-asyncio (`asyncio_mode = "auto"`) + fakeredis + pytest-timeout; ~3,200 passing tests (this figure is always the PASSING count, not the collected one) plus two opt-in integration tiers (testcontainers): a 99-test `pg` tier and a 49-test `redis` tier; coverage gate `fail_under = 80` (actual ~96%) |
...
| `ytdl:stream:{webpage_url}` | string | ≤30m (expire-capped) | probed-playable stream URL + `_STREAM_CACHE_FIELDS` metadata, plus a `traceparent` naming the extraction that minted the URL — the only record of where a serving URL came from, and what the playback span links back to |
...
| `LIVENESS_INTERVAL_SECS` | `15.0` | touch cadence, floored at 1.0s and refused non-finite (`_float_env`). Must stay well under the healthcheck's 90s staleness window; the two are written separately (Dockerfile ↔ config.py) and are not enforced by `just pins` |
...
| `NOW_PLAYING_UPDATE_INTERVAL_SECS` | `3.0` | NP progress-bar edit cadence. Floored at 1.0s and refused non-finite (`_float_env`): each tick is a PATCH per playing guild. The live elapsed label is floored to 10s so consecutive ticks render an identical payload and the edit is skipped |
| `QUEUE_MAX_ENTRIES` | `500` | ceiling on a guild's queue, in entries, counting the in-flight head (minimum 1). A `-play` that would cross it queues what fits and says so ("queued N of M — the queue is full at X"); one that fits nothing gets a notice. The same number caps the flat playlist extraction (`playlistend` in `_YTDL_PLAYLIST_OPTS`), so a link past it never resolves rows only to drop them. Read outside the queue mutex: `-play` is serialized per guild and the cap bounds spam, not an invariant. `interject()`'s own `put_front` is not capped |
