...
| `LIVENESS_FILE` | — (`/tmp/bot-alive` in the Dockerfile) | path a loop-resident task touches every `LIVENESS_INTERVAL_SECS`, read by the runtime image's `HEALTHCHECK`. A stale mtime (>90s) means the event loop wedged while the process stayed up, which `restart: always` cannot see — it observes only the process exiting. NOT a dependency probe: a Redis blip must not restart the container. Unset outside Docker, where the task never starts |
