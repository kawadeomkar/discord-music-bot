...
9. **Restoring the persisted queue and *playing* it are separate concerns.** `loop()` waits on the playback gate (`_playback_gate`) after `_restore_complete` and before its first dequeue; the gate opens only once a voice connection exists — in `start()` when the guild already has a voice client (the crash-recovery path connects first), otherwise from `-join`/`-play`. This is load-bearing: `cog_before_invoke` builds a `MusicPlayer` for *every* command, including ones `validate_commands` is about to reject, and an ungated loop walks the persisted queue and discards it against a nonexistent voice client. Never open the gate from a path that has not confirmed the handshake, and never bypass `defer_playback()` when inserting ahead of a restored queue — `-play` holds the gate across the join it triggers so the restored head cannot start before the requested song is inserted in front of it. Anything inserting into the queue on that path must `await mp.wait_for_restore()` first: `put_front()` LPUSHes the Redis mirror while `restore_entries()` replays already-listed entries in memory only, so an earlier insert is queued twice. See `docs/PLAYBACK_GATE_PLAN.md`.
