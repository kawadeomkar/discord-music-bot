...
| Tests | pytest + pytest-asyncio (`asyncio_mode = "auto"`) + fakeredis + pytest-timeout; ~2,830 tests plus two opt-in integration tiers (testcontainers): a 74-test `pg` tier and a 41-test `redis` tier; coverage gate `fail_under = 80` (actual ~94%) |
...
| `LIVENESS_FILE` | — (set to `/tmp/bot-alive` by the Dockerfile) | path a loop-resident task touches every `LIVENESS_INTERVAL_SECS`, read by the runtime image's `HEALTHCHECK`. A stale mtime (>90s) means the event loop wedged while the process stayed up, which `restart: always` cannot see — it only observes the process exiting. Deliberately NOT a dependency probe: a Redis blip must not restart the container. Unset (the default outside Docker) skips the task, and nothing reads the file there |
