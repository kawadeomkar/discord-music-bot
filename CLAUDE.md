...
12. **Redis eviction policy is `volatile-lru` on purpose.** Three keys carry no TTL
    ...
    `guild:{id}:config` holds a guild's DURABLE choices (debug mode, volume, timezone, idle and alone timeouts, progress-bar refresh) — evicting it silently reverts a setting the guild chose, with no log
