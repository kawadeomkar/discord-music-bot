...
| `YTDLP_WORKER_MEMORY_MB` | `1024` | `RLIMIT_DATA` each extraction worker applies to itself in `_worker_init` (`0` lifts it): a runaway response body fails as a `MemoryError` inside one job — classified as an `ExtractionError` naming the page — instead of the kernel killing the worker and breaking every in-flight future. `RLIMIT_DATA`, NOT `RLIMIT_AS`: rlimits are inherited, and yt-dlp's Deno child aborts under a 1 GiB address-space limit while running under the same data limit. The compose `mem_limit` on the bot service is the backstop. See `docs/ARCHITECTURE.md#extraction-bounds` |
| `YTDLP_EXTRACT_TIMEOUT_SECS` | `60.0` | Ceiling on ONE yt-dlp extraction, enforced on both sides of the process boundary: a SIGALRM inside the worker stops the work (yt-dlp's `socket_timeout` is per-recv and a slow-drip server never trips it), and the caller waits this plus a 5s grace as the backstop for a job the signal cannot interrupt. Both render as one `timed_out` `ExtractionError`. Floored at 5s via `_float_env`. See `docs/ARCHITECTURE.md#extraction-bounds` |
