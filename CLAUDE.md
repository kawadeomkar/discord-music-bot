...
6. **Version pins move in lockstep.** Bump both halves in the same commit. `just pins`
   ...
   **Five pairs are NOT enforced — this list is what a maintainer checks by hand,
   ...
   it below 60s and every healthy container reports unhealthy. No build compares them.
...
12. **Redis eviction policy is `volatile-lru` on purpose.** Four keys carry no TTL
    ...
    `guild:{id}:config` holds a guild's DURABLE choices (debug mode, volume, timezone, idle and alone timeouts, progress-bar refresh, lookup notice) — evicting it silently reverts a setting the guild chose, with no log
    ...
    the startup orphan sweep, which the `writer_app_id` stamp makes safe on a Redis
    shared with another bot — so it scales with guild count and not with runtime.
...
13. **The chat-command surface is not a SemVer API.** Adding, renaming or REMOVING a
    ...
    deleted under this rule in the release this branch ships, and `--now` stopped
    collapsing a playlist to its first track in the same one. A removal still owes users an
...
- Layout: one `tests/test_<module>.py` per src module. A command's tests live with its
  ...
  there are one behavior; and `test_musicbot.py` owns `play_placement.py`, since what
  decides where a song lands and what the channel is told about it are one behavior),
...
The operator can override twelve of these at runtime with `-settings bot <setting> <value>`
...
`DEBUG_TICK_SECS`/`DEBUG_DEADLINE_SECS`, `ANALYTICS_RENDER_DEADLINE_SECS`), within a chat
