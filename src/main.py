...
class MusicContext(commands.Context):
    ...
    async def send(
        ...
        # ≤10 is Discord's per-message embed cap (worst case here is 4: the block, a
        # playlist card and its unavailable-songs notice).
...
class MusicBotApp(commands.AutoShardedBot):
    ...
    async def setup_hook(self) -> None:
        ...
        self._bot_settings_hydration = asyncio.create_task(
            self.bot_settings.hydrate_until_read()
        )
    ...
    async def on_ready(self) -> None:
        ...
        # The startup read retries on its own; one that returned unread (no
        # application id then) starts again here.
        ...
        if (
            ...
            self._bot_settings_hydration = asyncio.create_task(
                bot_settings.hydrate_until_read()
            )
