...
from src.history_archive import HistoryArchive, PostgresHistoryArchive, _sanitize_entry
    ...
    async for key in redis.scan_iter(match=_HISTORY_KEY_MATCH, count=100):
        ...
        for start in range(0, total, page):
            ...
            for wire in raw:
                ...
                # Sanitized here even though PostgresHistoryArchive.insert_batch
                # sanitizes again. It looks like duplicate work (~7.5us/entry on
                # a one-shot tool) and it is — but `archive` is the HistoryArchive
                # PROTOCOL, which promises nothing about sanitizing, and legacy
                # rows are the likeliest source of poison in the whole system.
                # Dropping this would make the backfill correct only for one
                # implementation.
