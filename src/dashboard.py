...
async def run_live_dashboard(
    ...
    *,
    ...
    try:
        ...
        # the whole deadline, so the displacement is visible. A third cost, cheaper
        # to pay: bypassing ctx.send also bypasses debug-mode decoration, so callers
        # that want the footer pre-render one and thread it through `render`.
