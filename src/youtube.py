...
async def _run_extract(
    ...
    the pool binding and _ytdlp_extract's positional argument order live in one
    place.
    ...
    return await ytdlp_pool.run(_ytdlp_extract, url, opts, download, process)
    ...
    async def prefetch_stream(
        ...
        try:
            ...
            data = cast(
                ...
                await _run_extract(qo.webpage_url, _YTDL_STREAM_OPTS),
            )
        ...
        *,
        ...
        try:
            data = await _run_extract(
                search, _YTDL_STREAM_SEARCH_OPTS, download=download
            )
