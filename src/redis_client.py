...
async def _read_config_batches(
    ...
    *,
    ...
    """(guild id, reply) for each guild whose batch was read; a batch that fails
    or outlives `batch_timeout` is left out, with one WARNING. Pipelined rather
    than one awaited command each: the pool RAISES rather than queueing past its
    cap, so a plain fan-out fails every guild past it."""
...
async def read_config_writers(
    ...
    """Each guild's CONFIG_WRITER_FIELD stamp, on read_guild_configs' contract: a
    guild whose batch failed or timed out is absent. So is a guild whose key
    carries no integer stamp."""
    replies = await _read_config_batches(
        redis,
        guild_ids,
...
_BOT_CONFIG_FIELDS: Final[frozenset[str]] = frozenset(
...
class GuildRedisStore:
    ...
    async def get_config(self) -> GuildConfig:
        ...
        A caller that must tell the two apart uses read_config."""
    ...
    async def read_config(self) -> Optional[GuildConfig]:
        ...
        "could not read" stays distinguishable from "never chose"."""
        raw = cast(dict[bytes, bytes], await self.redis.hgetall(self.config_key()))
        return GuildConfig.from_redis(raw)
...
class BotConfigStore:
    ...
    async def read_config(self) -> Optional[BotConfig]:
        """The stored overrides, or None when the read failed."""
        raw = cast(dict[bytes, bytes], await self.redis.hgetall(self.config_key()))
    ...
    async def reset_config_fields(self, *fields: BotConfigFieldName) -> bool:
        ...
        if not fields or not _BOT_CONFIG_FIELDS.issuperset(fields):
            log.warning(
