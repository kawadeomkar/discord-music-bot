...
class GuildRedisStore:
    ...
    async def refresh_ttl(self) -> None:
        """Refresh GUILD_TTL on the TTL-managed guild keys — history included,
        like _pipe_expire_all. Since the Phase C cutover the history key rides
        idle expiry (a display-cache seed; Postgres holds full history), so the
        restore/stop refresh that backs this must re-arm it with the rest."""
        try:
            pipe = self.redis.pipeline()
            self._pipe_expire_all(pipe)
            await pipe.execute()
