...
def join_succeeded(ctx: commands.Context) -> bool:
    """Did the join a cold-start command ran leave a USABLE voice client?
    is_connected(), not the type alone: discord.py registers the client BEFORE the
    handshake completes, and join() swallows its own failures, so a failed one
    arrives as an absent client. Shared by -play and -resume."""
    vc = ctx.voice_client
