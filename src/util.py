...
# Discord's embed field-value cap. It rejects the WHOLE send past this, and the
# -remove reply carrying one of these lands after the songs are already gone from
# memory and Redis — so the user reads "Command failed" for a removal that
# happened, with no undo. Bounded here rather than per caller, because every
# caller composes the same shape and only this function knows the total.
...
def queue_message(songs: list[str]) -> str:
    ...
    Ten entries was never a bound on its own: each line is a title or a link echo,
    so ten 100-char titles compose to 1040 — over the cap before anything unusual
    happens. Overflow of either kind ends in the same trailing mark; the count the
    reader needs is in the embed's own title, not here."""
    lines: list[str] = []
    used = 0
    ...
    for i, song in enumerate(songs[:10]):
        ...
        if used + len(line) > budget:
            # A single line over budget is truncated rather than dropped: an empty
            # field would say nothing at all about what was taken.
            if not lines:
