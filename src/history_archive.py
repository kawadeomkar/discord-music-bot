...
- PostgresHistoryArchive — the asyncpg implementation. Lazily connects on first
  ...
  backoff loop absorbs failures instead). It applies no DDL: the schema is owned
  by migrations/ and src/db_migrate.py, and this verifies the version (_ensure).
- HistoryOutboxDrainer — the background task moving entries from the Redis outbox
  STREAM to Postgres: replay this consumer's pending IDs → read new ones →
  INSERT ... ON CONFLICT DO NOTHING → XACK+XDEL by ID. At-least-once delivery; a
  crash between insert and ack redelivers and play_history_dedup collapses the
  replay. The playback loop never awaits Postgres — add() XADDs and notify()s.
  One task per process, but not one per DEPLOYMENT: the outbox is a consumer
  group, so two live drainers are safe by construction and there is no lease to
  win — `>` hands them disjoint entries and the pending replay hands them a
  shared set that ON CONFLICT collapses.
...
The outbox is non-evictable by design, so anything that stops the drain grows a
Redis key that eventually refuses every write in the process, not just history's.
Each guard closes one way that could happen:

  poison entry   an entry Postgres will never accept blocks the batch behind it
                 forever → prevented at construction (HistoryEntry.__post_init__
                 clamps into the column domain). _isolate is the backstop for a
                 validator regression or schema drift, parking what it cannot
                 insert in play_history_rejected
  ...
  two drainers   structural — XACK settles the IDs this process archived, so a
                 second drainer cannot retire entries it never inserted
  tombstone      an entry whose body was deleted while still pending replays as an
                 ID with no payload; treating it as an entry raises KeyError out of
                 every handler and stalls the drain forever → _settle_tombstones
                 acks and logs it as a lost play
  stranded PEL   entries under a consumer name nothing reads (a renamed consumer),
                 which the pending replay cannot reach → periodic XAUTOCLAIM sweep
...
class PostgresHistoryArchive:
    ...
    async def insert_batch(self, entries: Sequence[HistoryEntry]) -> None:
        ...
        asyncpg cannot be handed a value it will not encode. That is the schema lock,
        asserted directly in the pg tier by
    ...
    async def record_rejection(
        ...
        Always reachable at the moment it is needed: a REJECTION means the server
        is up and said no to this row, so the same connection that carried the
        refusal carries this insert. An OUTAGE never reaches here — the entry stays
        on the outbox and redelivers. There is no third case.
        ...
        the orjson bytes verbatim, which is what makes replay exact.

        `wire` is those DELIVERED bytes, and passing them is what makes "verbatim"
        true. Re-serializing the parsed entry looks equivalent and is not: during a
        mixed-version rollout an entry written by a NEWER build carries fields this
        build's parser drops, so the dead letter would record a lossy re-encoding
        of the refused row — and, because the dedup identity is an md5 of payload,
        the same entry seen by two build versions would land as two rows in a table
        whose whole contract is that a row count is a failure count. Falls back to
        re-serializing when the caller has no wire bytes.
        """
    ...
    async def health_check(self) -> None:
        ...
        Connects if nothing has yet: the pool is lazy, so before the first song end
        it is simply absent, and reporting "not configured" for an enabled tier
        would be a lie an operator acts on. Pool creation is bounded by _ensure's
        timeout=10; -ping's own deadline (PING_DEADLINE_SECS, 3s) is shorter, so a
        hung first connect renders as FAILED rather than DOWN — both red.

        A benign case worth knowing before someone "fixes" that gap: the very first
        -ping of a bot that has not finished a song pays the whole connect here, so
        a healthy-but-slow connect (cold container, DNS, TLS) can render ONE red
        row for a database that is fine. It self-corrects on the next tick, and
        widening the probe deadline would delay every OTHER row's first render.

        SELECT 1 rather than a table read: this asks "is the server up and
        answering", not "is the schema right" — _ensure's version check already
        settled that, and an unmigrated database fails here with its message.
        """
        pool = await self._ensure()
        async with pool.acquire(timeout=_ACQUIRE_TIMEOUT_SECS) as conn:
...
# later". Since the schema lock (HistoryEntry.__post_init__) this set is expected
# to be UNREACHABLE — every outbox entry was clamped into the play_history column
# domain at construction. It survives as the backstop for a validator regression
# or a schema this build was not written for, because dropping a whole batch on
# either turns a one-row bug into a hundred-play loss.
#
#   DataError            SQLSTATE 22xxx, server-side data rejections:
#                        CharacterNotInRepertoireError for NUL bytes,
#                        NumericValueOutOfRangeError, …
#   CheckViolationError  SQLSTATE 23514, and NOT a DataError — both inherit from
#   NotNullViolationError IntegrityConstraintViolationError. Without these two
#                        arms a CHECK violation would propagate past the isolate
#                        path and wedge the drain head permanently.
#
# Deliberately NOT here — adding any of these breaks the drain:
...
#     problems and for values its encoders cannot encode, but so does an ordinary
#     bug anywhere inside insert_batch — naming them would dead-letter a healthy
#     batch on a refactor.
...
class HistoryOutboxDrainer:
    ...
    async def _drain_batch(self) -> int:
        ...
        of its DATA — with one deliberate exception, guild_id 0, which __post_init__'s
        docstring explains it declines to fix up. A refusal here therefore means a
    ...
    async def _isolate(self, batch: list[OutboxEntry], span: Span) -> None:
        """One batch refused: retry it row by row so one bad row costs one row, not
        the 99 batched with it. Expected to be dead code — it exists because
        dropping the whole batch on a validator regression turns a one-row bug into
        a hundred-play loss.
        ...
        - Settle per entry, never once at the end. A transient error partway
          through raises before an end-of-batch settle would run, redelivering rows
          already recorded as rejected. Per entry also makes the pass RESUMABLE,
          which matters because single inserts cost ~22x a batch (measured: 15.7ms
          for executemany(100) vs 343ms for 100 singles), so on a degraded server
          this can exceed DRAIN_DEADLINE_SECS and be cancelled.
        - Iterate the batch, not what parsed. A corrupt element parses to None;
          settling only what parsed would leave it delivered-and-unacked forever,
          which is the wedge this path exists to prevent.

        A transient error simply raises out of here, leaving the current entry and
        everything after it to redeliver. Duplicate rejection rows are prevented by
        _REJECT_SQL's ON CONFLICT, not by exclusivity: a stable consumer name lets
        two drainers hold the same poisoned batch via the pending replay and both
        write the same refused row. That matters because play_history_rejected is
        expected to stay empty forever and `just db-rejects` reports its contents —
        two rows must mean two poisoned entries, never one seen twice.
        """
        rejected = 0
    ...
    async def _enforce_cap(self) -> None:
        ...
        Dropping un-archived plays is data loss, full stop — hence the unbounded
        default and the ERROR on every drop. It exists for operators who would
        rather lose the oldest history than have a long Postgres outage push a
        non-evictable key into Redis' maxmemory, where it stops *every* write in
        the process.

        NEVER RUNS WHILE SHUTTING DOWN: a departing process has the least
        information about what a live peer is doing. Checked once per pass, so
        stop() also halts a convergence loop already in progress.

        ACK BEFORE TRIM. XTRIM does not consult the PEL: on real Redis 7.4.9, five
        delivered-and-unacked entries survived `XTRIM MAXLEN 2` as pending records
        while three bodies were destroyed. Trimming first leaves an ID pending with
        no body — a tombstone, which replays every cycle forever. A crash between
        the two leaves entries acked but not trimmed, which is invisible to readers
        and reclaimed by the next pass.

        The cap DELIBERATELY destroys entries a drainer is holding. Refusing to
        cross the PEL sounds safer and is the one thing that would make this method
        useless: during a Postgres outage — the scenario HISTORY_OUTBOX_MAX exists
        for — the drain cycle re-reads the same pending batch every tick, so the
        OLDEST entries are permanently in flight and a clamp below the oldest
        pending ID would trim nothing while the backlog grew without bound. Their
        drainer already holds the parsed rows in memory, so an insert that
        subsequently succeeds still archives them.

        PAGED, looping until depth is back at the cap. The trim is one MINID
        command however large the tranche, and the ack's ID set is bounded by
        BATCH_SIZE x live drainers because the drain cycle never reads `>` with a
        non-empty PEL. The minid discovery is the leg that scales with the backlog:
        XRANGE has no ID-only form, so its reply carries bodies, and a single
        COUNT=overage fetch against a deep backlog would run outside
        DRAIN_DEADLINE_SECS with nothing bounding it. Hence CAP_PAGE — each pass
        fetches one bounded page, trims that tranche and re-checks depth.
        """
