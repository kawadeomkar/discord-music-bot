...
async def _in_dm(
    ...
    if isinstance(parsed, SettingsRequest):
        if parsed.scope is SettingScope.BOT or parsed.action is SettingsAction.SHOW:
...
def _bot_card(cog: MusicBot) -> discord.Embed:
    ...
    return card.bot_card(
        ...
        unread=bot_settings is not None and bot_settings.unread,
    )
...
async def _bot_scope(
    ...
    *,
    ...
    if request.action is SettingsAction.DETAIL:
        ...
        unread = bot_settings is not None and bot_settings.unread
        await _send(ctx, card.detail(spec, shown, default=None, read_failed=unread))
        return
    ...
    if bot_settings.ignore_stored:
        ...
        return
    # Read before the write, which does not wait for the stored settings to land.
    unread = bot_settings.unread
    ...
    if result.previous is not None:
        before = format_value(spec, result.previous)
    else:
        before = "?" if unread else "env"
...
async def _change_server(
    ...
    if request.action is SettingsAction.SET:
        ...
        dual = named(spec.key, SettingScope.BOT) is not None
