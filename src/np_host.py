...
class NpHost:
    ...
    def __init__(
        ...
        # Last payload pushed and the host it went to, for the caller's no-op-edit
        # guard. Held here because it dies with the pointer: a retire strip-edits
        # the message outside that guard, so a cache outliving its host would
        # suppress the edit that repairs it. Opaque to this module — compared only
        # for equality, so the element type stays Any.
        self._last_rendered: Optional[list[Any]] = None
    ...
    def render_is_current(self, message_id: int, rendered: list[Any]) -> bool:
        """True when `rendered` is what was last pushed to this exact message, so
        the caller can skip the edit. Both halves are the test: the bar changes ~10
        times in a 4-minute song while the 3s tick fires ~80."""
        return rendered == self._last_rendered and message_id == self._last_id

    def note_render(self, message_id: int, rendered: list[Any]) -> None:
        """Record a payload that LANDED. Never call it for an edit that raised —
        caching what failed to push suppresses the retry that fixes it."""
