...
class MusicBot(commands.Cog):
    ...
    async def _command_error(
        ...
        if detail is None:
            if isinstance(
                e, (ExtractionError, SpotifyRateLimitError, SpotifyRequestError)
            ):
    ...
    async def queue_source(
        ...
        ResolvedSpotifyStream for a Spotify album or playlist (its page-1 fetch is
        already in flight — no ordering decision is made here, so starting the HTTP
        call early is safe), a ResolvedYoutubePlaylist (already resolved in full), or
        a bare QueueObject."""
        if isinstance(source, SpotifySource) and source.type in (
            ...
            return ResolvedYoutubePlaylist(tracks, playlist_url=source.playlist_url)
        else:
    ...
    async def _enqueue_playlist(
        ...
        *,
        ...
        Spotify collections stream page-by-page via _begin_stream_enqueue instead."""
    ...
    async def _resolve_playnow_source(
        ...
        The Spotify path consumes page 1 of the stream and abandons the generator
