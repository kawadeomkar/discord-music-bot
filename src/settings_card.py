...
STORED_ZONE_UNUSABLE: Final = "stored zone unusable"
...
_BOT_CARD_UNREAD: Final = (
    "⚠️ Couldn't read the stored bot settings yet, so a stored value may show as its "
    "environment value. Try again in a moment."
)
_CARD_UNREAD: Final = (
    "⚠️ Couldn't read this server's saved settings just now, so some may show "
    "their default. Try again in a moment."
)
...
def server_shown(
    ...
    *,
    ...
    value under its write-time minimum renders that minimum, which is what runs, as
    the card's range prints it, and names the stored value: `5s`,
    `bot minimum; set here 3s`."""
    raw = getattr(stored, spec.field) if stored is not None and spec.field else None
    if spec.kind is SettingKind.TIMEZONE and isinstance(raw, str):
        # A stored name this host cannot resolve runs on the default zone, and is
    ...
    if (
        ...
        and (floor := write_range(spec)[0]) is not None
        and value < floor
    ):
...
def _add_rows(
    ...
    """One field per group, in SettingGroup's order, spilling into "(cont.)" before
    a field outgrows Discord's limit."""
    for group in SettingGroup:
        if not any(spec.group is group for spec, _ in rows):
            continue
...
def _server_command(spec: SettingSpec) -> str:
    return f"-settings {card_name(spec)}"
...
def _bot_command(spec: SettingSpec) -> str:
    # The registry key, which is also what -settings bot <setting> takes.
    return f"-settings bot {spec.key}"


def bot_card(
    *, rows: list[tuple[SettingSpec, Shown]], ignored: bool, unread: bool = False
) -> discord.Embed:
    """The bot-wide settings, for the operator only, in the server card's shape."""
    description = (
        "Bot-wide: each applies in every server, except where a server's own choice "
        "takes precedence, and only the bot's operator can change them."
    )
    if ignored:
        description = (
    ...
    if unread:
        description = f"{_BOT_CARD_UNREAD}\n\n{description}"
    embed = discord.Embed(
        title="Bot settings",
        description=description,
        color=DEGRADED_COLOR if unread else CHANGE_COLOR,
    )
...
def detail(
    ...
    *,
    ...
    already names its baseline. `read_failed`: a stored value may be missing from
    the current one, for a bot setting because no read has applied them yet."""
    ...
    if spec.scope is SettingScope.BOT and spec.env is not None:
        parts.append(f"Environment variable `{spec.env}`")
    ...
    unread = _BOT_CARD_UNREAD if spec.scope is SettingScope.BOT else _CARD_UNREAD
    return discord.Embed(
        description=f"{unread}\n\n{text}." if read_failed else f"{text}.",
        color=DEGRADED_COLOR if read_failed else CHANGE_COLOR,
    )
...
_DEBUG_FOOTER_LABEL: Final = server_spec(ConfigField.DEBUG_MODE).label
...
def bot_set_reply(
    ...
    *,
    ...
    replaced, or None when the knob ran on its environment value. `unread`: no read
    has applied the stored settings yet, so with no override what it replaced is
    unknown, and the reply names nothing."""
    if previous is not None:
        was = f" (was **{format_value(spec, previous)}**, set by the bot's operator)"
    elif unread:
        was = ""
    else:
        baseline, env = _baseline(spec)
        source = f"from `{env}`" if env else "the default"
        was = f" (was **{format_value(spec, baseline)}**, {source})"
    text = (
        f"**{spec.label}** is now **{format_value(spec, value)}** {_bot_reach(spec)}"
        f"{was}. It applies {spec.applies}. "
        f"{_BOT_SAVED if persisted else _NOT_SAVED} {_changed_by(mention)}"
    )
    return notice_embed(text, CHANGE_COLOR)


def _bot_reach(spec: SettingSpec) -> str:
    """Where a bot setting's new value applies. A server setting of the same key
    either replaces it or, with a write minimum that follows it, only slows it."""
    server = named(spec.key, SettingScope.SERVER)
    if server is None:
        return "for every server"
    if server.write_minimum is not None:
        return "for every server, and a server's own can only be slower"
    return "for every server that has not set its own"
...
def debug_default_reply(
    ...
    *,
    ...
    if following == 1:
        unchosen = "the **1** server that has not chosen for itself"
    else:
        unchosen = f"the **{following}** servers that have not chosen for themselves"
    if value is None:
        text = f"**{label}**'s default is back to `DEBUG_MODE`: **{host}**, for {unchosen}."
    elif value:
        keep = "" if host_default else "; set `DEBUG_MODE=true` to keep it on"
        text = (
            f"**{label}** is now **on** by default, for {unchosen} (this bot is in "
            f"**{total}**). Every "
        ...
        others = total - following
        if others == 1:
            kept = "; the other **1** keeps its own choice"
        elif others:
            kept = f"; the other **{others}** keep their own choice"
        else:
            kept = ""
        text = (
            f"**{label}** is now **off** by default, for {unchosen}{kept}. It lasts "
            f"until the bot restarts, which returns to `DEBUG_MODE` (**{host}**)."
        )
