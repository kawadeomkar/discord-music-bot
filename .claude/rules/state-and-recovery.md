...
| `spotify:{track,artist,album}:{id}`, `spotify:playlist:v3:{id}`, `spotify:album_tracks:v1:{id}` | string | 24h/24h/24h, 1h, 24h | cached lookups. A playlist entry is a dict of the kept titles plus the playlist's name, total length and unavailable-item count, and only a complete walk writes one; an album entry is the same dict plus its artists and cover. The key is versioned by that VALUE's shape: under the 1h TTL an unversioned key would answer a deploy that changes it with the previous build's entries |
