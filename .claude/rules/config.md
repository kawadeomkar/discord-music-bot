...
| `GUILD_READY_TIMEOUT_SECS` | `0.5` | how long discord.py waits for ANOTHER `GUILD_CREATE` before declaring the shard ready (its own default is `2.0`). Its `_delay_ready` loop exits ONLY by timing out, so the whole value is charged to every start whatever the guild count — and `on_ready`, which starts the per-guild recovery fan-out, cannot fire until it does. A guild that misses the window is still dispatched by `parse_guild_create` and still in `bot.guilds`, so recovery covers it either way. Raise it if a guild is ever missing from `bot.guilds` at `on_ready`. Read once, at `MusicBotApp` construction. Between 0.1 and 10: a value outside refuses startup, naming the variable |
