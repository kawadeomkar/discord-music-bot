...
class TestSetupHook:
    ...
    async def test_no_postgres_url_leaves_durable_tier_off(
    ...
    async def test_postgres_url_starts_durable_tier(
        self, app: MusicBotApp, monkeypatch: pytest.MonkeyPatch
    ) -> None:
...
class TestClose:
    ...
    async def test_stops_drainer_then_database_then_pool(
