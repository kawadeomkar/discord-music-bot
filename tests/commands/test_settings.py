...
from tests.helpers import command_callback, described, recording_span
...
@pytest.fixture
async def bot_settings(cog: MusicBot, fake_redis_bot: Redis) -> BotSettings:
    """What setup_hook builds on MusicBotApp, on the cog's fake Redis, once its
...
def span() -> Iterator[MagicMock]:
    with recording_span() as current:
        yield current
...
class TestScopes:
    ...
    async def test_the_card_says_when_stored_settings_are_not_read_yet(
        self, cog: MusicBot, settings_ctx: MagicMock, bot_settings: BotSettings
    ) -> None:
        bot_settings.hydrated = False
        ctx = _as_operator(settings_ctx)
        await _invoke(cog, ctx, "bot")
        assert described(_embed(ctx)).startswith(
            "⚠️ Couldn't read the stored bot settings yet"
        )
        await bot_settings.hydrate()
        await _invoke(cog, ctx, "bot")
        assert not described(_embed(ctx)).startswith("⚠️")

    async def test_an_unread_store_is_named_on_a_detail_and_a_write(
        self,
        cog: MusicBot,
        settings_ctx: MagicMock,
        bot_settings: BotSettings,
        ...
        bot_settings.hydrated = False
        ctx = _as_operator(settings_ctx)
        await _invoke(cog, ctx, "bot heartbeat")
        assert described(_embed(ctx)).startswith(
            "⚠️ Couldn't read the stored bot settings yet"
        )
        with caplog.at_level(logging.INFO, logger="src.commands.settings"):
            await _invoke(cog, ctx, "bot heartbeat 10")
        assert "(was" not in _text(ctx)
        assert "settings: bot heartbeat ? -> 10s" in caplog.text
...
class TestDebugDefault:
    ...
    def servers(self, cog: MusicBot) -> list[MagicMock]:
        ...
        cog.debug_settings.apply_choices({GUILD + 2: False})
        return guilds
    ...
    async def test_on_names_the_servers_it_reaches_and_what_they_show(
        ...
        assert _text(settings_ctx) == (
            "**Debug footer** is now **on** by default, for the **2** servers that "
            "have not chosen for themselves (this bot is in **3**). Every embed in them — "
    ...
    async def test_off_and_reset(
        ...
        assert _text(ctx) == (
            ...
            "have not chosen for themselves; the other **1** keeps its own choice. "
...
class TestCard:
    ...
    async def test_the_operators_card_points_at_the_bot_card_and_carries_none(
        ...
        embed = _embed(settings_ctx)
        assert described(embed).endswith("`-settings bot` shows bot-wide settings.")
...
class TestRefusedAndFailedWrites:
    async def test_a_write_refused_by_a_concurrent_forget_says_nothing_changed(
        self, cog: MusicBot, settings_ctx: MagicMock
    ) -> None:
        ctx = _with_manage_server(settings_ctx)
        ctx.guild.id = GUILD
        refused = WriteResult(applied=False, persisted=False, previous=None)
        with patch.object(GuildSettings, "write", new=AsyncMock(return_value=refused)):
            await _invoke(cog, ctx, "leave-when-idle 10m")
        assert _text(ctx) == card.not_applied().description
        ctx.send.assert_awaited_once()

    async def test_a_body_that_raises_becomes_the_error_embed(
        self, cog: MusicBot, settings_ctx: MagicMock
    ) -> None:
        with patch("src.commands.settings.run", side_effect=RuntimeError("boom")):
            await _invoke(cog, settings_ctx)
        assert _embed(settings_ctx).title == "Command failed"
