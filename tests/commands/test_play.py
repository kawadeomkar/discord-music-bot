...
class TestPlaynowWrapper:
    """`-playnow` is `-play --now` under another name. It survives as a spelling
    users already know; it must not become a second implementation, so every row
    asserts the flag route's own seams were driven."""
    ...
    async def test_it_interjects_a_live_song(
        self, music_bot: MusicBot, mock_ctx: MagicMock
    ) -> None:
        seams = self._wire(music_bot, mock_ctx, live=True)
        mock_ctx.voice_client = self._vc(playing=True)
        with no_typing("src.commands.play.background_typing"):
            await command_callback(MusicBot.playnow)(music_bot, mock_ctx, url="s")
        seams.interject.assert_awaited_once()
        seams.command_error.assert_not_awaited()

    async def test_with_nothing_playing_it_enqueues_like_play(
        self, music_bot: MusicBot, mock_ctx: MagicMock
    ) -> None:
        """The fall-through the old command had, now inherited from the flag route:
        `--now` with nothing to interrupt places rather than failing."""
        seams = self._wire(music_bot, mock_ctx, live=False)
        mock_ctx.voice_client = self._vc()
        with no_typing("src.commands.play.background_typing"):
            await command_callback(MusicBot.playnow)(music_bot, mock_ctx, url="s")
        seams.interject.assert_not_awaited()
    ...
    async def test_the_flag_never_reaches_the_query(
        ...
        """The origin is what -remove matches on. A prepended flag left in it would
        persist a user_input the user never typed."""
        seams = self._wire(music_bot, mock_ctx, live=False)
        mock_ctx.voice_client = self._vc()
        with no_typing("src.commands.play.background_typing"):
            await command_callback(MusicBot.playnow)(
                music_bot, mock_ctx, url="never gonna give you up"
            )
        assert (
    ...
    async def test_a_flag_typed_anyway_is_not_searched_for(
        ...
        """`-playnow --next x` is still now. Prepending alone would leave `--next`
        in the query and search for it as text."""
        seams = self._wire(music_bot, mock_ctx, live=True)
        mock_ctx.voice_client = self._vc(playing=True)
        with no_typing("src.commands.play.background_typing"):
            await command_callback(MusicBot.playnow)(
                music_bot, mock_ctx, url="--next some song"
            )
        seams.interject.assert_awaited_once()
        assert "--next" not in seams.interject.await_args.args[1]
