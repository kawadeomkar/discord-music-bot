...
class TestYtdlpLogger:
    ...
    def test_unified_search_opts_carry_stream_format(self):
...
class TestYTSourceUnifiedExtraction:
    ...
    async def test_always_extracts_with_process_true(self, mock_ctx):
    ...
    async def test_fresh_extraction_writes_both_caches(self, mock_ctx, fake_redis):
    ...
    async def test_fresh_extraction_populates_full_metadata(self, mock_ctx):
...
class TestExtractionPool:
    """The yt-dlp extraction pool lifecycle (_get_pool / prewarm / shutdown).

    The session-wide conftest fixture pins src.youtube._YTDLP_POOL to an in-process
    ThreadPoolExecutor, so every test here saves and restores that global rather than
    leaking a real ProcessPoolExecutor into the rest of the suite.
    """

    def _swap_pool(self, value):
        """Set the module pool global, returning the previous value to restore."""
        import src.youtube as youtube

        previous = youtube._YTDLP_POOL
        youtube._YTDLP_POOL = value
        return previous

    def test_get_pool_lazily_creates_one_process_pool(self):
        import src.youtube as youtube

        previous = self._swap_pool(None)
        # Patch the constructor so the test never touches real multiprocessing —
        # it asserts the wiring (created once, sized from _POOL_WORKERS, memoized),
        # not that a subprocess spawns.
        sentinel = MagicMock(name="ProcessPoolExecutor-instance")
        try:
            with patch(
                "src.youtube.ProcessPoolExecutor", return_value=sentinel
            ) as ctor:
                first = youtube._get_pool()
                second = youtube._get_pool()
            assert first is sentinel
            assert second is sentinel  # memoized — created once, then reused
            ctor.assert_called_once_with(
                max_workers=youtube._POOL_WORKERS,
                initializer=youtube.configure_worker_logging,
            )
        finally:
            self._swap_pool(previous)

    def test_prewarm_is_noop_for_thread_pool(self):
        import src.youtube as youtube

        pool = MagicMock()
        previous = self._swap_pool(pool)
        try:
            youtube.prewarm_ytdlp_pool()
            # A non-ProcessPoolExecutor has nothing to spawn — no submits.
            pool.submit.assert_not_called()
        finally:
            self._swap_pool(previous)

    def test_prewarm_submits_one_noop_per_worker(self):
        import src.youtube as youtube
        ...
        try:
            youtube.prewarm_ytdlp_pool()
            assert pool.submit.call_count == youtube._POOL_WORKERS
            for call in pool.submit.call_args_list:
                assert call.args[0] is youtube._pool_warmup_noop
        finally:
            self._swap_pool(previous)

    def test_shutdown_joins_and_clears_the_pool(self):
        import src.youtube as youtube

        pool = MagicMock()
        previous = self._swap_pool(pool)
        try:
            youtube.shutdown_ytdlp_pool()
            pool.shutdown.assert_called_once_with(wait=True, cancel_futures=True)
            assert youtube._YTDLP_POOL is None
        finally:
            # Restore the session's thread pool — leaving it None would make the next
            # _get_pool() in this worker build a real ProcessPoolExecutor and break
            # tests that submit an unpicklable MagicMock extractor.
            self._swap_pool(previous)
    ...
    def test_shutdown_is_safe_when_no_pool_exists(self):
        ...
        try:
            ...
            youtube.shutdown_ytdlp_pool()  # must not raise
            assert youtube._YTDLP_POOL is None
        finally:
            self._swap_pool(previous)

    async def test_run_extract_heals_a_broken_pool_and_retries_once(self):
        """A worker death (BrokenProcessPool) must not brick extraction: _run_extract
        rebuilds the pool and retries once, succeeding on the second attempt."""
        from concurrent.futures import ThreadPoolExecutor
        from concurrent.futures.process import BrokenProcessPool

        import src.youtube as youtube

        # Global stays None so _discard_pool()'s rebuild path has nothing real to tear
        # down; _get_pool is patched so neither attempt constructs a ProcessPoolExecutor.
        previous = self._swap_pool(None)
        pool = ThreadPoolExecutor(max_workers=1)
        try:
            with (
                patch.object(youtube, "_get_pool", return_value=pool) as get_pool,
                patch.object(
                    youtube,
                    "_ytdlp_extract",
                    side_effect=[BrokenProcessPool("worker died"), {"ok": 1}],
                ) as extract,
                patch.object(youtube, "_discard_pool") as discard,
            ):
                result = await youtube._run_extract("u", {}, False, True)
            assert result == {"ok": 1}
            assert extract.call_count == 2  # first raised, retry succeeded
            discard.assert_called_once()  # the broken pool was dropped before retry
            assert get_pool.call_count == 2  # a fresh pool fetched for the retry
        finally:
            pool.shutdown(wait=False)
            self._swap_pool(previous)

    async def test_run_extract_propagates_a_second_broken_pool(self):
        """If the pool breaks again on the retry, the error propagates — _run_extract
        heals exactly once, it doesn't loop forever."""
        from concurrent.futures import ThreadPoolExecutor
        from concurrent.futures.process import BrokenProcessPool
        ...
        try:
            with (
                patch.object(youtube, "_get_pool", return_value=pool),
                patch.object(
                    youtube,
                    "_ytdlp_extract",
                    side_effect=BrokenProcessPool("worker died"),
                ),
                patch.object(youtube, "_discard_pool"),
            ):
                with pytest.raises(BrokenProcessPool):
                    await youtube._run_extract("u", {}, False, True)
        finally:
            pool.shutdown(wait=False)
            self._swap_pool(previous)
