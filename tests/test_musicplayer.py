...
class TestPrefetchSettling:
    ...
    async def test_the_slot_outlives_the_cancel_it_guards(
        ...
        — reads an empty slot, skips its own await on this task and takes a SECOND
        claim while this one is still open. Two live claims settle by position, so
        each takes the other's song: one queued song retired unplayed, another
        played twice, deque and mirror in step the whole way."""
