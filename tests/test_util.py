...
class TestSendQueuePhrases:
    async def test_pineapplecat_receives_special_phrase(self) -> None:
        ctx = MagicMock()
        ctx.send = AsyncMock()
    ...
    async def test_bryan_receives_insult(self) -> None:
        ctx = MagicMock()
        ctx.send = AsyncMock()
    ...
    async def test_other_user_receives_nothing(self) -> None:
        ctx = MagicMock()
        ctx.send = AsyncMock()
