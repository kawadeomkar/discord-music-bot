...
class TestPushQueue:
    ...
    async def test_refreshes_ttl_on_history_key(
        self, store: GuildRedisStore, fake_redis: aioredis.Redis
    ) -> None:
...
class TestPushHistory:
    ...
    async def test_trims_to_display_window(
        self, store: GuildRedisStore, fake_redis: aioredis.Redis
    ) -> None:
    ...
    async def test_history_key_gets_idle_expiry(
        self, store: GuildRedisStore, fake_redis: aioredis.Redis
    ) -> None:
        await store.push_history(_hentry(1))
        assert await fake_redis.ttl(store.history_key()) > 0

    async def test_lazily_demotes_pre_cutover_key(
        self, store: GuildRedisStore, fake_redis: aioredis.Redis
    ) -> None:
...
class TestRefreshTtl:
    ...
    async def test_refreshes_history_key_ttl(
        self, store: GuildRedisStore, fake_redis: aioredis.Redis
    ) -> None:
        # Post-cutover the history key rides idle expiry with the rest of the
        # guild keys (docs/POSTGRES_HISTORY_PLAN.md §5.3) — refresh_ttl, which
        # backs the restore/stop refresh, must re-arm it too.
        await fake_redis.lpush(store.history_key(), b'"entry"')
        await store.refresh_ttl()
        assert await fake_redis.ttl(store.history_key()) > 0
