...
class TestCacheGet:
    ...
    async def test_returns_none_on_cache_miss(self, fake_redis: Redis) -> None:
        result = await cache_get(fake_redis, "nonexistent:key")
        assert result is None

    async def test_returns_decoded_value_on_hit(self, fake_redis: Redis) -> None:
...
class TestCacheSet:
    async def test_sets_value_with_ttl(self, fake_redis: Redis) -> None:
...
@pytest.fixture
def store(fake_redis: Redis) -> GuildRedisStore:
...
class TestPushQueue:
    async def test_rpush_adds_item(
...
class TestGetQueue:
    async def test_returns_all_items_oldest_first(
        self, store: GuildRedisStore, fake_redis: Redis
    ) -> None:
    ...
    async def test_returns_empty_list_when_key_missing(
...
class TestPushHistory:
    ...
    async def test_trims_to_50(self, store: GuildRedisStore, fake_redis: Redis) -> None:
        for i in range(55):
...
class TestGetHistory:
    async def test_returns_up_to_50_items(
        self, store: GuildRedisStore, fake_redis: Redis
    ) -> None:
        for i in range(60):
...
class TestSetState:
    async def test_writes_field_to_hash(
        self, store: GuildRedisStore, fake_redis: Redis
    ) -> None:
...
class TestGetState:
    async def test_returns_hash_fields(
        self, store: GuildRedisStore, fake_redis: Redis
    ) -> None:
...
class TestRefreshTtl:
    async def test_refreshes_all_guild_keys(
        self, store: GuildRedisStore, fake_redis: Redis
    ) -> None:
...
class TestClearConnection:
    async def test_removes_channel_and_song_fields(
        self, store: GuildRedisStore, fake_redis: Redis
    ) -> None:
        await fake_redis.hset(store.state_key(), b"voice_channel_id", b"111")
        await fake_redis.hset(store.state_key(), b"text_channel_id", b"222")
