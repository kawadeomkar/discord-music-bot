...
class TestResolveModeThreading:
    ...
    async def _resolve(
        ...
        with patch("src.musicbot.YTDL.yt_source", new=spy):
            await music_bot.queue_source(
                mock_ctx, source, analytics=_ANALYTICS, origin=_ORIGIN, mode=mode
            )
        return spy
    ...
    def test_every_placement_may_resolve_flat(self) -> None:
        """The policy has one name so it has one test. Interjection is not a
        Placement — it never reaches this function."""
        for placement in Placement:
            assert resolve_mode_for(placement) is ResolveMode.FLAT_OK
    ...
    async def test_an_interjection_head_resolves_full(
        ...
        with patch.object(music_bot, "queue_source", new=spy):
            await music_bot._resolve_interjection_source(
                mock_ctx, parse_input("take on me"), origin=_ORIGIN
            )
        assert spy.await_args is not None
...
class TestHistoryCommand:
    ...
    def test_the_command_is_capped_at_one_render_per_guild(self, name: str) -> None:
        ...
        `-play` is not in this list: it admits through PlayRegistry, which caps a
        guild's in-flight requests and serializes only the insert.
...
class TestClearCommand:
    ...
    async def test_an_unrestored_queue_is_never_cleared(
        ...
        """clear() destroys the Redis mirror while reading the IN-MEMORY display, so
        against an unrestored player it deletes a saved queue it cannot see, and an
        interjection stack loses its rows with it. A cold player is reachable:
        validate_commands only requires the AUTHOR in voice."""
        mp = MagicMock()
        mp.queue_clear = AsyncMock(return_value=[])
...
def _admit(
    ...
    *,
    ...
    truthy mock, which place() reads as torn down; pin it as _mock_mp() does."""
...
class TestNowFlagRouting:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            # join_succeeded because the mocked join leaves no voice client.
...
class TestPlayCommand:
    ...
    async def test_the_warm_path_hands_queue_source_the_placement_mode(
        ...
        assert music_bot.queue_source.await_args.kwargs["mode"] is (
            resolve_mode_for(Placement.TAIL)
        )
    ...
    async def test_the_cold_path_hands_queue_source_the_placement_mode(
        ...
        assert music_bot.queue_source.await_args is not None
        assert music_bot.queue_source.await_args.kwargs["mode"] is (
            resolve_mode_for(Placement.COLD_FRONT)
        )
...
class TestPlayFrontInsertion:
    ...
    async def test_cold_path_routes_playlist_through_front(
        ...
        """End-to-end wiring for the playlist half of the cold path: play()'s list
        branch carries the COLD_FRONT placement into _enqueue_playlist, and a
        playlist front-inserts in full."""
        mock_ctx.voice_client = None
        tracks = [
            QueueObject(f"https://yt.com/v={i}", f"Track {i}", mock_ctx.author)
            for i in range(3)
...
class TestEnqueueSingle:
    ...
    async def test_warning_gets_its_own_message_on_the_repin_path(
        ...
        await music_bot._enqueue_single(
            mock_ctx, qobj, mp, _admit(music_bot, mock_ctx, mp), warning="watch out"
        )
    ...
    async def test_the_note_survives_the_repin_path(
        ...
        await music_bot._enqueue_single(
            mock_ctx, qobj, mp, _admit(music_bot, mock_ctx, mp), note="3 more follow"
        )
...
class TestEchoIsSafeInAnEmbed:
    ...
    def test_the_echo_is_bounded_well_inside_the_field_cap(self) -> None:
        ...
        # A LITERAL bound, and tight: safe_label caps BEFORE escaping, so 200 raw
        # chars become at most 400 plus the ellipsis, and ten of these must fit one
        # 1024-char field.
...
class TestCommandArgumentBinding:
    """`-play` and `-remove` both consume the rest of the line: a positional binds
    ONE WORD, so `-play never gonna give you up` would record `"never"` as the
    origin and `-remove never` would be a wildcard. Asserted on the callback
    signature, since the binding is a property of the signature."""
...
class TestNowFlag:
    ...
    def live_mp(self) -> MagicMock:
        """A MusicPlayer mock with a song playing, built on _mock_mp(): the not-live
