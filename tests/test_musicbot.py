...
class TestEnqueueSingle:
    ...
    async def test_next_inserts_through_queue_put_next_and_names_what_it_waits_on(
        ...
        And no "Est. playing at": the ETA walk seeds from the current song's FULL
        duration as a proxy for what is left of it, which is fine at the back of a
        queue and badly wrong for the very next slot. Naming the song it waits
        behind is exact."""
