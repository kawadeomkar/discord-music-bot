...
The two per-guild TTL-less keys have exactly one deletion path: `GuildRedisStore.clear_guild`, called from `on_guild_remove` after the guild's player (if any) is torn down through `cleanup()`. It DELs the guild's six keys — state, queue, now_playing, history, config, recovery lock — in one command. A guild the bot has left is the one case where retaining its history and choices forever serves nobody; the outbox is global and is not touched, so a play recorded during the teardown still reaches the archive.
