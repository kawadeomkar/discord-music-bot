...
`removed` is the LREM shortcut and **only a removal may pass it**: LREM asserts the survivors kept their order, which is false for a shuffle and for any insert. The two writes scale with different sets — a rebuild rewrites what **survives** (measured on `redis:7-alpine`: 0.77 ms at 50, 3.75 ms at 800, 24.5 ms at 4800), LREM touches only what **goes** (0.96 ms for one entry at depth 1000, against ~5.7 ms to rebuild there) — so the crossover is a **ratio, not a count**. It sits near `drop = survivors / 4`; `_LREM_MAX_SHARE` gates at one in five, which is where every measured point falls on the right side. A count-only gate is depth-blind and gets this wrong in the feature's own headline case: at depth 250 dropping 200 it picks LREM at 4.7 ms against a 0.8 ms rebuild. `_LREM_MAX_ENTRIES` (200) is a second, absolute bound for deep queues, where a quarter of the survivors is still hundreds of LREMs — **not** the ~40 an earlier estimate gave, which came from fakeredis, where a pipeline costs what its commands cost rather than one round trip.
