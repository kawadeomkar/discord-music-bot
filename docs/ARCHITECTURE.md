...
`src/queue_rows.py` renders one queued item as a row of text, and owns the ETA walk that runs down a listing. It is pure: no player, no queue, no Discord call. A listing reads the same wherever it appears because there is one formatter, and it has three callers: `-queue` (`MusicPlayer.queue_embed`, two-line rows), the queued album and playlist cards (`MusicPlayer.queued_rows`, one-line rows), and the single-entry "Up next" and "Queued song" cards, which read the same fields:
