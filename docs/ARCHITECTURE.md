...
| `QUEUE_MAX_ENTRIES` | No | Ceiling on a guild's queue, in entries, counting the in-flight head (default `500`, minimum `1`). A `-play` that would cross it queues what fits and says so in its confirmation; one that fits nothing gets a "queue is full" notice. The same number caps the flat playlist extraction (`playlistend`), so a link past it never resolves rows only to drop them — which also means an `&index=` past the cap reports the playlist as having that many songs. Read outside the queue mutex: `-play` is serialized per guild and the cap bounds spam rather than guarding an invariant, so a racing `-playnow` fallback insert may overshoot by its own batch. `interject()`'s own `put_front` is not capped — an interjection replaces the playing song with itself and one tail, and each costs a 1–4 s resolve first. |
...
| `_YTDL_PLAYLIST_OPTS` | `yt_playlist` | `noplaylist: False`, `extract_flat: "in_playlist"`, `playlistend: QUEUE_MAX_ENTRIES` — enumerates entries without per-video extraction, and no more of them than the queue could hold |
...
- **Nearly-finished guard**: a song with almost no time left gets no resume entry (`resume_position is None`) — it just ends. Only a known duration can be near its end.
- **No duration, no offset**: a song whose `duration_secs` is 0 (a live stream, or a VOD missing its metadata) still parks a tail, but with no `ts` and so no `-ss` — a seek into a live source decodes and discards that many seconds of live audio as silence. The tail returns at the live edge (a VOD from its start); `InterjectOutcome.live` lets the confirmation say so. Crash recovery applies the same rule: `_restore_state` drops the recorded position when the parked duration is 0 or absent.
