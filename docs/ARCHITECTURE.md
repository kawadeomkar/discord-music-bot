...
- **The `next_up` card carries a thumbnail**, so the block shows two: the song's and the next song's. That is a consequence of the card being the `-play` confirmation — the confirmation has always had one, and dropping it for the block's copy would reintroduce the drift the shared renderer exists to prevent. `_build_queue_entry_embed` sets it only for a URL `is_web_url()` accepts, because a restored entry's thumbnail comes off the wire unvalidated and a bad one 400s every send and tick edit for that guild.
