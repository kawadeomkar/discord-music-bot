...
| `-restart` | `rs`, `replay` | — | Replay the live song from its beginning: a copy carrying no `ts` is front-inserted (`MusicPlayer.restart_current()` → `RestartOutcome`) and the song is stopped, so the loop's ordinary dequeue plays it again. The queue behind it is untouched, a paused song comes back paused, and the interrupted play keeps its own `-history` entry — unlike a `-playnow` tail, the replay spans none of what was already heard. |
