...
| `guild_state.py` | Schema module: **every byte persisted to Redis is defined here**. Field-name constants (`StateField`, `NowPlayingField`, `QueueEntryField`, `ConfigField`, `BotConfigField`, with `ConfigFieldName`/`BotConfigFieldName` as their `Literal` types) + frozen value objects (`GuildStateData`, `NowPlayingData`, `SongQueueEntry`/`SearchQueueEntry`, `GuildPlaybackSnapshot`, `HistoryEntry`, `GuildConfig`, `BotConfig`) with `from_redis`/`to_redis` converters. `GuildConfig` is the durable-settings object behind `guild:{id}:config`, and every one of its fields is `Optional` on purpose: absent means "follow the host default", which an explicit `False`/`0.0` does not (`tzinfo()` resolves the stored IANA name at read time, falling back to `DEFAULT_TIMEZONE` rather than raising on a render path). `CONFIG_DOMAIN` maps each numeric config field to a `ConfigDomain` (`lo`, `hi`, and whether `OFF_SECS` means off), and `GuildConfig.__post_init__` reads a value outside it as unset, with a WARNING, wherever a `GuildConfig` is built; the legacy `:state` volume parses through the same domain. `CONFIG_WRITER_FIELD` (`writer_app_id`) is a wire field beside the settings, not one of them: the application that last wrote the hash. `BotConfig` is `bot:{application_id}:config`, one `Optional` field per bot knob named as its env var lowercased; it only parses (a count is an exact `int()`), and the bounds stay with the `-settings` registry. `DEFAULT_IDLE_TIMEOUT_SECS`/`DEFAULT_ALONE_TIMEOUT_SECS` sit beside `DEFAULT_TIMEZONE`, because the `-settings` registry, the playback loop and the watchdog all read them and this is the one module that imports nothing from `src`. Pure data — no domain logic, no project runtime imports. Wire formats are pinned by golden-fixture tests. |
...
| `redis_client.py` | Connection-pool lifecycle + `GuildRedisStore` (per-guild Redis ops: queue/state/now-playing/history/config keys, pause epochs, recovery gate + lock, atomic start-song transaction). Every write to `guild:{id}:config` `PERSIST`s it and no path `EXPIRE`s it — that key is a guild's durable settings and is excluded from every shared TTL pipeline. Its config methods: `read_config` (None when the read failed, where `get_config` answers all-unset), the dedicated `set_volume`/`migrate_volume` (both refuse a volume outside `CONFIG_DOMAIN`), `set_debug_mode` and `set_timezone`, the generic `update_config` (refuses those three fields and an empty config), `reset_config_fields` and `reset_volume` (both copies, one MULTI); each setter stamps `writer_app_id` when passed `writer=`. `BotConfigStore` is the same shape for `bot:{application_id}:config` — `read_config`, `update_config`, `reset_config_fields` — on `@_bot_op`, `_guild_op`'s sibling. Module-level `cache_get`/`cache_set`, the outbox-stream helpers, `read_guild_configs` (pipelined, chunked — the per-guild fan-out it replaced exhausted the connection pool above `max_connections` guilds and reported the failures as "never chose"), `read_config_writers` (the stamps, on the same contract), `scan_guild_config_ids` (a bounded `SCAN`, None on any failure) and Spotify-token helpers. Every store method catches and logs Redis errors — Redis being down degrades persistence, never playback. |
...
| `settings_card.py` | `-settings`' embeds, pure like `analytics_card.py`: the server card (one prose line per setting, `**label** · value · source`, grouped as the registry groups them; code blocks wrap badly on a phone), the operator-only bot card (code-block rows in `-debug`'s Config form, `5s (bot owner; env 3s)`), one setting's detail, and every change and refusal reply. Values render through the registry's `format_value`, so a value copied off a card parses back. The bot card is never a second embed on `-settings`: a response in the player's home channel becomes the Now Playing host, whose embeds ride every progress tick. |
...
| `help.py` | `MusicHelpCommand` — a `commands.HelpCommand` subclass rendering the command list and per-command help as man(1)-styled embeds (NAME / SYNOPSIS / DESCRIPTION / EXAMPLES / NOTES). Per-command copy (`brief`/`help`/`usage`/`extras`) lives on the command declarations in `musicbot.py`; categories/order come from `CATEGORY_COMMANDS`. `extras["sections"]` is `(name, entries)` pairs a command builds from its own data, rendered after EXAMPLES through the same spill-into-"(cont.)" field writer as the command list: `-settings` passes the registry's `help_sections()`, so a server setting added to the registry reaches `-help settings` with no copy to update. `get_destination()` returns the `MusicContext` (not the bare channel) so help output routes through the NP-block attach path. |
...
- **Volume reaches the prefetched song**: a source's level is baked into FFmpeg's `-filter:a` when it is built, and the prefetch builds the next song one song early. So where the prefetched branch and the in-band resolve meet, before anything reads `current_song`, `loop()` compares `YTDL.volume` with `self.volume` and, when they differ, rebuilds the source over the same stream data (`_at_current_volume` → `YTDL.from_stream_data`) and reaps the old FFmpeg off the loop. The check awaits nothing and touches no claim, so the rebuilt source carries `persisted` and the commit is unchanged. It covers a completed prefetch, one still resolving and the in-band resolve alike, and costs one FFmpeg spawn only when a write landed after the source was built. The rebuilt source opens the URL the prefetch probed without probing it again; a URL revoked in between fails like any prefetched one. A spawn that fails keeps the old source, at its old level. A write that lands after the check and before `vc.play()` applies one song later.
...
- **Three readers fill the cache.** A restoring player seeds it from the snapshot it
  ...
  reaches the pool's cap), with a retry that backs off from 1s to 60s for any guild a
